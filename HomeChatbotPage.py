...
def render():
    client = OpenAI(
        ...
        api_key=st.secrets["FEATHERLESS_API_KEY"] # Using active balance
    )
    ...
    # --- Context Sync Notification [cite: 2026-03-02] ---
    if "last_profile" in st.session_state:
        ...
        st.warning("⚠️ Please complete your **Farmer Profile** first for personalized analysis.")
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing..."):
                try:
                    profile_info = st.session_state.get("last_profile", "No profile data loaded.")
                    system_instructions = f"""
                    You are a financial inclusion assistant. 
                    Context: {profile_info}
                    Language: {current_lang}
                    Answer specifically based on the farmer's profile data.
                    """
    ...
    st.markdown("---")
    st.markdown(f"<div style='text-align: center; color: #2E7D32;'>🌾 <strong>Farm Ledger Africa</strong> | Multi-Lingual AI ({current_lang})</div>", unsafe_allow_html=True)
